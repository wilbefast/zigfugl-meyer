...
% ------------------------------------------------------------------------------  
	...
	au développeur de s'abstraire des données brutes. 
	
	Le groupe OpenNI défini des normes auxquelles doivent obéir cette couche, de 
	manière à faciliter la communication à travers la pile de technologie. Parmi
	les bibliothèques peu-connues qui conformement à ses standards~:
  \begin{itemize}
  \item\emph{TopTep Skeletonize~:} permet de suivre le mouvement des doigts,
  \item\emph{GlobalSensing Technologies API~:} permet l'étiquetage et la 
  reconnaissance des postures.
  ...
  l'API de bas niveau et une chouche d'analyse des données qu'OpenNI appéleratit 
  middleware. 
  ...
  plus de resources et peut ammener à de faux positives.
  % http://labs.vectorform.com/2011/06/windows-kinect-sdk-vs-openni-2/
	...
	multi-platforme développé par PrimeSense. Vu que c'est PrimeSense qui a 
...
% ------------------------------------------------------------------------------  
  ...
  de gestes, un API permettant de créer des boutons, curseurs et d'autre 
  éléments GUI, contrôlés par le mouvement.
